            ...
            <div className="max-w-md mx-auto bg-[#ffff] p-8 mt-6 md:mt-10 lg:mt-16 shadow-2xl">
               ...
               <form onSubmit={handleSubmit(onSubmit)}>
                  <div className="form-control">
                     ...
                     <input type="text" {...register("name", { required: true })} placeholder="Name" className="input input-bordered h-[2.5rem] rounded-lg bg-slate-200 mb-2" />
                     {errors.name && <span className="text-red-600 text-[0.9rem]">Name is required</span>}
                  </div>
                  <div className="form-control">
                     <label className="label">
                  ...
                  <div className="form-control">
                     ...
                     <div className="relative">
                        <input type={showPassword ? 'text' : 'password'}  {...register("password", {
                           ...
                           maxLength: 20,
                           pattern: /(?=.*[A-Z])(?=.*[!@#$&*])(?=.*[0-9])(?=.*[a-z])/
                        })} placeholder="Password" className="input input-bordered h-[2.5rem] rounded-lg bg-slate-200 w-full mb-2" />
                        {errors.password?.type === 'required' && <p className="text-red-600 text-[0.9rem]">Password is required</p>}
                        {errors.password?.type === 'minLength' && <p className="text-red-600 text-[0.9rem]">Password must be at least 6 characters</p>}
                        {errors.password?.type === 'maxLength' && <p className="text-red-600 text-[0.9rem]">Password must be less than 20 characters</p>}
                        {errors.password?.type === 'pattern' && <p className="text-red-600 text-[0.9rem]">Password must have atleast one uppercase one lowercase, one number and one special character</p>}
                        <button
                           type="button"
